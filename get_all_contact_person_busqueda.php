...
$db = new DB_CONNECT();
            ...
            $query = "SELECT  A.COUNTRY AS COUNTRY , A.NAME AS NAMECENTER , B.NAME AS TYPE ,D.NAME AS AREA, F.KTYPENAME AS KTYPE, C.ID AS IDPERSON FROM yf_INSTITUTION A INNER JOIN  yf_TYPE B ON  A.TYPE=B.ID INNER JOIN yf_CONTACTPERSON C ON C.IDINST =  A.ID  INNER JOIN yf_AREA D ON C.AREA =  D.ID   INNER JOIN yf_PROJECT E ON E.IDCONTACT=C.ID  INNER JOIN yf_KTYPE F ON E.KTYPE=F.ID "." WHERE ".$countryb." AND B.ID ". $typeb  ." AND F.ID ".$ktypeb ." AND D.ID ".$areab. " ORDER BY A.COUNTRY";
